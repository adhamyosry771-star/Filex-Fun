...
const LoginView: React.FC<LoginViewProps> = ({ language, setLanguage, onGuestLogin }) => {
  ...
  return (
    <div dir={language === 'ar' ? 'rtl' : 'ltr'} className="relative h-screen w-full bg-gray-900 flex flex-col justify-end pb-12 items-center overflow-hidden font-sans">
      ...
      {/* Background Blobs */}
      <div className="absolute top-[-20%] left-[-20%] w-[500px] h-[500px] bg-brand-500/20 rounded-full blur-[100px]"></div>
      <div className="absolute bottom-[-10%] right-[-10%] w-[300px] h-[300px] bg-accent-500/20 rounded-full blur-[80px]"></div>

      {/* Language Toggle */}
      <button 
        onClick={toggleLanguage}
        className="absolute top-8 right-8 z-30 flex items-center gap-2 bg-white/5 px-4 py-2 rounded-full text-white text-xs font-bold hover:bg-white/10 transition border border-white/10"
      >
      ...
      <div className={`z-20 flex flex-col items-center w-full px-8 text-center transition-all duration-700 ${mode !== 'welcome' ? 'mb-4 scale-75' : 'mb-16'}`}>
        <div className="relative w-24 h-24 mb-6">
            <div className="absolute inset-0 bg-gradient-to-tr from-brand-500 to-accent-500 rounded-3xl rotate-12 opacity-80"></div>
            <div className="absolute inset-0 bg-gray-800 rounded-3xl border border-white/10 flex items-center justify-center shadow-2xl z-10">
        ...
        {mode === 'welcome' && (
            <div className="space-y-1">
                <p className="text-white text-lg font-bold">{t('slogan')}</p>
                <p className="text-brand-300 text-sm">{t('subSlogan')}</p>
            </div>
        )}
      </div>
      ...
      {mode === 'welcome' && (
          <div className="z-20 w-full max-w-sm px-6 space-y-4 animate-in slide-in-from-bottom-10 fade-in duration-500">
            <button 
              ...
              className="w-full bg-white text-gray-900 font-bold py-4 rounded-2xl flex items-center justify-center gap-3 hover:scale-105 transition shadow-lg relative overflow-hidden group"
            >
            ...
            <button 
              ...
              className="w-full bg-white/10 backdrop-blur text-white font-bold py-4 rounded-2xl flex items-center justify-center gap-3 hover:bg-white/20 transition border border-white/10"
            >
      ...
      {(mode === 'email_login' || mode === 'email_signup') && (
          <div className="z-20 w-full max-w-sm px-6 animate-in slide-in-from-bottom-10 fade-in duration-300">
              <div className="bg-gray-800 rounded-[2rem] p-8 border border-white/5 shadow-2xl">
                  ...
                  <div className="space-y-5">
                      <div className="relative group">
                          <Mail className="absolute top-3.5 left-4 rtl:right-4 rtl:left-auto w-5 h-5 text-gray-500 group-focus-within:text-brand-400 transition" />
                          <input 
                            type="email" 
                            placeholder={t('email')}
                            value={email}
                            onChange={e => setEmail(e.target.value)}
                            className="w-full bg-black/40 border border-gray-600 rounded-xl py-3.5 px-12 text-white focus:border-brand-500 focus:outline-none transition"
                          />
                      </div>
                      <div className="relative group">
                          <Lock className="absolute top-3.5 left-4 rtl:right-4 rtl:left-auto w-5 h-5 text-gray-500 group-focus-within:text-brand-400 transition" />
                          <input 
                            type="password" 
                            placeholder={t('pass')}
                            value={password}
