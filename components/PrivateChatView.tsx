...
const PrivateChatView: React.FC<PrivateChatViewProps> = ({ language, onBack, currentUser, chatSummary }) => {
  ...
  const handleSend = async () => {
      ...
      await sendPrivateMessage(currentUser.uid, chatSummary.otherUserUid, inputText);
      setInputText('');
  };
                ...
                return (
                    <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[75%] px-4 py-2 rounded-2xl text-sm ${
                            isMe ? 'bg-brand-600 text-white rounded-br-none' : 'bg-gray-800 text-white rounded-bl-none'
                        }`}>
                            {msg.text}
